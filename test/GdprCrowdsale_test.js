...
contract("GdprCrowdsale", accounts => {
  ...
  context("crowdsale", () => {
    ...
    it("distributed the initial token amounts correctly", async () => {
      ...
      const marketingBalance = await tokenContract.balanceOf.call(
        marketingPool
      );
    ...
    it("allows token purchases", async () => {
      ...
      const saleInitialBalance = await web3.eth.getBalance(
    ...
    it("does not allow presales after the ico has started", async () => {
      await assertThrows(saleContract.addPresaleOrder(buyer4, 5000));
    });
