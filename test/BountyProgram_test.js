...
contract("GdprCash", accounts => {
  ...
  context("Bounty Program", () => {
    ...
    it("can give approval to the owner for transfers", async () => {
      ...
      const marketingAllowance = await tokenContract.allowance(
        marketing,
        owner
      );
    ...
    it("owner can make transfers even before the crowdsale end", async () => {
      ...
      await tokenContract.transferFrom(experts, userA, sendAmount, {
        from: owner
      });
      const userBalanceAfter = await tokenContract.balanceOf.call(userA);
      assert.equal(
        userBalanceAfter.toNumber() - userBalanceBefore.toNumber(),
        sendAmount
      );
    });
    ...
    it("owner cannot transfer more than allowed", async () => {
      ...
      const sendAmount = 2;
      await assertThrows(
        tokenContract.transferFrom(experts, userA, sendAmount, { from: owner })
      );
    });
  });
});
