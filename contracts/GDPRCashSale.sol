 ...
 * tokens (GDPR). It does so by converting ETH received into a quantity of
 * tokens that are transferred to the contributor via the ERC20-compatible
 * transferFrom() function.
 */
contract GDPRCashSale is Pausable {
    ...
    /**
     * Constructor for a crowdsale of GDPR Cash tokens.
     *
     ...
     * @param rateGDPRToEther                the conversion rate from GDPR to Ether
     * @param addressOfTokenUsedAsReward    address of the token being sold
     */
    function GDPRCashSale(
        ...
        setRate(rateGDPRToEther);
        tokenReward = GDPRCash(addressOfTokenUsedAsReward);
    }
    ...
    function () public payable whenNotPaused beforeDeadline afterStartTime saleNotClosed nonReentrant {
        ...
        // and GDPR have the same number of decimal places (18)
    ...
    /**
     * The owner can update the rate (GDPR to ETH).
     *
     * @param _rate  the new rate for converting GDPR to ETH
     */
     ...
     * are in units of wei and mini-GDPR. Every digit counts.
     *
     * @param _to            the recipient of the tokens
     * @param amountWei     the amount contributed in wei
     * @param amountMiniGDPR the amount of tokens transferred in mini-GDPR
     */
    function ownerAllocateTokens(address _to, uint amountWei, uint amountMiniGDPR) external onlyOwner nonReentrant {
        if (!tokenReward.transferFrom(tokenReward.owner(), _to, amountMiniGDPR)) {
            revert();
        }
    ...
    */
    ...
    /**
     * Given an amount in GDPR, this method returns the equivalent amount
     * in mini-GDPR.
     *
     * @param amount    an amount expressed in units of GDPR
     */
    function convertToMiniGDPR(uint amount) internal constant returns (uint) {
